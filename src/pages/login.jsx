...
import PrivacyAgreementModal, { hasAgreedToPrivacy, checkUserAgreedToTerms } from '../components/PrivacyAgreementModal';
...
export default function LoginPage() {
  ...
  const handleAgreementAgree = () => {
    ...
    const role = pendingAuth.role || pendingAuth.user?.role;
    authLogin(pendingAuth.user, pendingAuth.token);
    setSent(true);
    setShowAgreement(false);
    // Mark as first-time login complete
    localStorage.setItem('hasCompletedFirstLogin', 'false');
    setPendingAuth(null);
    setTimeout(() => {
      if (role === 'admin') {
        ...
        // For students on first login, go to meal preferences
        navigate('/meal-preferences', { replace: true, state: { isFirstLogin: true } });
      }
    }, 300);
  };
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      // If user hasn't agreed to privacy terms, show modal and pause login
      if (!checkUserAgreedToTerms(data.user)) {
        setPendingAuth({ user: data.user, token: data.token, role: userRole });
        setShowAgreement(true);
        setLoading(false);
        return;
      }

      // Proceed immediately if already agreed
      authLogin(data.user, data.token);
      setSent(true);
      notify.loginSuccess(data?.user?.name || 'User');
      setTimeout(() => {
        if (userRole === 'admin') {
          navigate('/admin', { replace: true });
      ...
      notify.loginError("Something went wrong. Please try again.");
    ...
    <>
      ...
      {showAgreement && (
        <PrivacyAgreementModal
          ...
          userName={pendingAuth?.user?.full_name || pendingAuth?.user?.name}
